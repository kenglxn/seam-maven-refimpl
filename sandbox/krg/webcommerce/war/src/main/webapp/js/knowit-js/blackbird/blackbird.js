...
( function() {
    ...
    /*
     * LOO-20081021: Modificatoins
     */
		content = timestampFunc() + (( content.constructor == Array ) ? content.join( '' ) : content);
    /* LOO-20081021: end */
  ...
  var logLevel = 31; // 11111 == all levels, 00000 == none
  ...
  function assignTimestampFunction() {
    return ( timeStamp 
      ? function() { 
        var now = new Date(), 
            d   = timeStamp & timeStamps.DATE, 
            t   = timeStamp & timeStamps.TIME,
            m   = timeStamp & timeStamps.MILLIS;
        
        return ( '<font color="cyan">[' + 
          ( d ? now.getFullYear() + '-' + now.getMonth() + '-' + now.getDate() : '' ) + 
          ( t ? (d ? ' ' : '') + now.toLocaleTimeString() : '' ) + 
          ( m ? (d || t ? ',' : '') + now.getMilliseconds() : '' ) + ']</font> ' );
        } 
      : function(){ return ''; } );
  };
  
  var timestampFunc = assignTimestampFunction();
  /* LOO-20081021 end */

  
  /*
   * LOO-20081022: New code
   */

  function initBb() {
  
    if (arguments.callee.done) {
      // Method has already been executed
      return; 
    }
    ...
    // IE does not like bc init calls initBb    
    //window[ NAMESPACE ].init = function() { if( !isVisible() ) show(); };
  ...
  window[ NAMESPACE ] = {
    ...
    logLevels: logLevels,
    addLogLevels: 
      function( bits ) { return( bits ? logLevel |= bits : logLevel = 0 ); },
    removeLogLevels: 
      function( bits ) { return( bits ? logLevel ^= bits : logLevel = 0 ); },
    ...
    addTimestamps:
      function( bits ) { 
        timeStamp = bits ? timeStamp | bits : 0;
        timestampFunc = assignTimestampFunction();  
        return timeStamp; 
      },
    removeTimestamps:
      function( bits ) { 
    ...
    isDebugEnabled:
      function() { return logLevel & logLevels.DEBUG; },
    isInfoEnabled:
      function() { return logLevel & logLevels.INFO; },
    isWarnEnabled:
      function() { return logLevel & logLevels.WARN; },
    isErrorEnabled:
      function() { return logLevel & logLevels.ERROR; },
    isProfileEnabled:
      function() { return logLevel & logLevels.PROFILE; },
    /* LOO-20081021 end */
     ...
     */
    debug: 
      function( msg ) { if( logLevel & logLevels.DEBUG ) addMessage( 'debug', msg ); },
    warn:
      function( msg ) { if( logLevel & logLevels.WARN )  addMessage( 'warn', msg ); },
    info:
      function( msg ) { if( logLevel & logLevels.INFO )  addMessage( 'info', msg ); },
    error:
      function( msg ) { if( logLevel & logLevels.ERROR ) addMessage( 'error', msg ); },
    profile: 
      function( label ) {
        if( logLevel & logLevels.PROFILE ) {
     ...
     */
    ,init:
      function() { initBb(); if( !isVisible() ) show(); }
    /* LOO-20081022 end */
  ...
  /*
   * LOO-20081022: Modificatoins
   */
  addEvent( window, 'load', function() { initBb(); });
  /* LOO-20081022 end */
