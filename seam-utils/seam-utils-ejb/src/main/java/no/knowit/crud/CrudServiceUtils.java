...
public class CrudServiceUtils {
  ...
  public static List<Object> getIdValues(final Object entity) {
    ...
    final List<Member> id = getIdAnnotations(entity.getClass());
    if(id.size() < 1) {
      throw new IllegalArgumentException(
          "Could not get identity. No @Id annotation found on class: " + entity.getClass());
    }
    final List<Object> result = new ArrayList<Object>(id.size());
  ...
  public static Map<String, Member> findQueryableAttributes(final Class<?> entityClass) {
    ...
    final Map<String, Member> attributes = new HashMap<String, Member>();
  ...
  public static Map<String, Member> reduceQueryableAttributesToPopulatedFields(
    ...
    final Map<String, Member> populatedAttributes = new HashMap<String, Member>();
    for (final Entry<String, Member> entry : attributes.entrySet()) {
      final String propertyName = entry.getKey();
      final Member member = entry.getValue();
      final Object value = member != null ? ReflectionUtils.get(member, exampleEntity) : null;
   ...
   */
  public static String createJpql(final Object exampleEntity, final boolean select,
  ...
  public static String createJpql(final Object exampleEntity, final Map<String, Member> attributes,
      final boolean select, final boolean distinct, final boolean any) {
    ...
    boolean where = false;
    final String operator = any ? "OR" : "AND";
    ...
    for (final Entry<String, Member> entry : properties) {
      ...
      if (value != null) {
        final Class<?> type = value.getClass();
        if (type != null) {
          final int k = OBJECT_PRIMITIVES.indexOf(type);
          if (type.isPrimitive() || type.isEnum() || k > -1) {
            // k=0 => java.lang.String
            final String equals = k == 0
            ? (value.toString().indexOf('%') > -1 ? "LIKE" : "=") : "=";

            if (!where) {
              where = true;
              jpql.append(String.format(" WHERE e.%s %s :%s", propertyName, equals, propertyName));
            } else {
              jpql.append(String.format(" %s e.%s %s :%s",
                  operator, propertyName, equals, propertyName));
            }
    ...
    for (Class<?> clazz = entityClass; clazz != Object.class; clazz = clazz.getSuperclass()) {
      if(!ignore(clazz)) {
        for(final Method method : clazz.getDeclaredMethods()) {
          final String propertyName = ReflectionUtils.getPropertyName(method);
